...
{
	// https://github.com/team-waldo/InsomniacArchive/blob/spiderman_pc/InsomniacArchive/Serialization/ObjectSerializer.cs
	// https://github.com/Tkachov/dat1_extractor/blob/main/dat1lib/types/sections/__init__.py
	...
	public class SerializedSection : Section
	{
		...
		private NodeType DetermineSignedIntNodeType(Int64 min, Int64 max) {
		...
		private NodeType DetermineUnsignedIntNodeType(UInt64 min, UInt64 max) {
			...
			switch (value.Type) {
				...
				case JTokenType.Array:
					...
					if (item != null && item.Type == JTokenType.Integer) {
						...
						Int64 min = (long)item;
						UInt64 max = (ulong)item;
						foreach (var it in arr) {
							Int64 sit = (Int64)it;
							UInt64 uit = (UInt64)it;
							if (sit < min) min = sit;
							if (uit > max) max = uit;
						}
		...
		private void SerializeObject(BinaryWriter w, JObject obj) {
			...
			foreach (var prop in properties) {
				...
				w2.Write((UInt32)hash);
				w2.Write((UInt16)flags);
				w2.Write((byte)0);
			...
			w.Write((UInt32)0);
			w.Write((UInt32)0x03150044);
			w.Write((UInt32)string_offsets.Count);
			w.Write((UInt32)childrenData.Length);
		...
		private void SerializeNode(BinaryWriter w, JToken node, NodeType itemType) {
			switch (itemType) {
				...
				case NodeType.UINT16: w.Write((UInt16)node); break;
				case NodeType.UINT32: w.Write((UInt32)node); break;
				case NodeType.INT8: w.Write((sbyte)node); break;
				case NodeType.INT16: w.Write((Int16)node); break;
				case NodeType.INT32: w.Write((Int32)node); break;
				...
				case NodeType.INSTANCE_ID: w.Write((UInt64)node); break;
		...
		private void SerializeString(BinaryWriter w, string str) {
			...
			w.Write((UInt32)bts.Length);
			w.Write((UInt32)CRC32.Hash(str, false));
			w.Write((UInt64)CRC64.Hash(str));
			w.Write(bts);
			w.Write((byte)0);
			Pad(w, 4);
		}
